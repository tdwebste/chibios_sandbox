...
#define print(msg, ...) chprintf((BaseSequentialStream*)&SDU1, msg, ##__VA_ARGS__)
...
void codec_hw_init(void)
{
	...
	codec_writeReg(0x99, 0x99);
	codec_writeReg(0x80, 0x47);
	codec_writeReg(codec_readReg(0x32) | (1 << 7), 0x32);
	codec_writeReg(codec_readReg(0x32) & ~(1 << 7), 0x32);
	...
	// Set beep time to C6 at 1.2 S
	codec_writeReg(0x1C, 0x83);

	// Set HP gain to 1.1430 and MSTBMUTE|MSTAMUTE = 0
	codec_writeReg(0x0d, 0xe3);
}
...
static void codec_dma_init(void)
{
	...
	if (!b)
		print( "DMA Allocated Successfully to I2S3\r\n");
...
void codec_writeReg(uint8_t addr, uint8_t data)
{
	...
	txbuf[0]=addr;
	txbuf[1]=data;
	ret = i2cMasterTransmitTimeout(&I2CD1, CS43L22_ADDR, txbuf, 2, NULL, 0, MS2ST(4));
	if (ret == RDY_OK)
		//print("I2C 0x%x -> 0x%x\r\n", data, addr);
		do {} while (0);
	else
